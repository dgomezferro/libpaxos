...
#define GET_PRO_INSTANCE(I) (&proposer_state[(I) % PROPOSER_ARRAY_SIZE])
...
// Handle infinite prepare requests
struct inf_prepare_info {
    int				infinite_opened;
    unsigned int    promises_bitvector;
    unsigned int    promises_count;
    i_status        status;
...
struct inf_prepare_info inf_prepare_info;
...
static int
handle_infinite_prepare_ack(prepare_ack * pa, short int acceptor_id) {
    // If not p1_pending, drop
    if(inf_prepare_info.status != p1_pending) {
        LOG(DBG, ("Promise dropped, infinite prepare not requested\n"));
        return 0;
    }

    // If not our ballot, drop
    if(pa->ballot != inf_prepare_info.my_ballot) {
    ...
    //Ack from already received!
    if(inf_prepare_info.promises_bitvector & (1<<acceptor_id)) {
        LOG(DBG, ("Dropping duplicate promise from:%d, infinite prepare request\n", acceptor_id));
        return 0;
    }

    // promise is new
    inf_prepare_info.promises_bitvector |= (1<<acceptor_id);
    inf_prepare_info.promises_count++;
    LOG(DBG, ("Received valid promise from:%d, infinite prepare request\n", acceptor_id));

    //Not a majority yet for this instance
    if(inf_prepare_info.promises_count < QUORUM) {
        LOG(DBG, ("Not yet a quorum for infinite prepare request\n"));
        return 0;
    }

    //Quorum reached!
    inf_prepare_info.infinite_opened = 1;
    inf_prepare_info.status = p1_ready;

    int active_count = p1_info.pending_count + p1_info.ready_count;
	unsigned int to_open = PROPOSER_PREEXEC_WIN_SIZE - active_count;

    p1_info.ready_count += to_open;

    LOG(DBG, ("Quorum for infinite prepare request reached\n"));
...
handle_prepare_ack_batch(prepare_ack_batch* pab) {
    ...
    for(i = 0; i < pab->count; i++) {
        ...
        if(pa->iid == INF_PREPARE_REQ){
        	// Infinite prepare ack
        	ready += handle_infinite_prepare_ack(pa, pab->acceptor_id);
        } else {
        	// Normal prepare ack
