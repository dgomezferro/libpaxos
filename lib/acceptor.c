...
//The infinite request record
static acceptor_record inf_request_record;
...
static acceptor_record *
acc_apply_infinite_prepare(prepare_req * pr) {
	acceptor_record * rec;

    //We already have a more recent ballot
    if (inf_request_record.is_final && inf_request_record.ballot >= pr->ballot) {
        LOG(DBG, ("Prepare infinite request dropped (ballots curr:%u recv:%u)\n",
    ...
    //Record not found or smaller ballot
    // in both cases overwrite and store
    LOG(DBG, ("Prepare infinite request is valid (ballot:%u)\n",
        pr->ballot));

    //Record does not exist yet
    rec = &inf_request_record;
    rec->iid = -1;
	rec->ballot = pr->ballot;
	rec->value_ballot = 0;
	rec->is_final = 1;
...
handle_prepare_req_batch(prepare_req_batch* prb) {
    ...
    for(i = 0; i < prb->count; i++) {
        ...
        if(pr->iid == INF_PREPARE_REQ) {
            //Try to apply infinite prepare
            rec = acc_apply_infinite_prepare(pr);
        } else {
